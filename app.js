...
async function generateQR() {
  ...
  for(const clsDoc of clsSnap.docs) {
    ...
    html += `<p><b>${clsDoc.data().className}:</b> ${token}</p>`; // FIXED template literal
...
async function loadEditableClasses() {
  ...
  snap.forEach(d => {
    const c = d.data();
    box.innerHTML += `<p><input id="class-${d.id}" value="${c.className}"> <button onclick="updateClass('${d.id}')">Update</button></p>`; // FIXED template literal
  });
}

async function updateClass(classId) {
  const val = document.getElementById(`class-${classId}`).value; // FIXED missing backticks
...
async function loadCorrections() {
  ...
  snap.forEach(d => {
    const data = d.data();
    box.innerHTML += `<p>Student: ${data.studentId} | Class: ${data.classId} <button onclick="approveCorrection('${d.id}')">Approve</button></p>`; // FIXED template literal
  });
}
