...
// Hide all teacher & student sections first
...
snap.forEach(d => {
editClasses.innerHTML +=    <p>   <input id="class-${d.id}" value="${d.data().className}">   <button onclick="updateClass('${d.id}')">Update</button>   </p>;
});
}
...
let q = currentUser.role === "teacher"
...
if (snap.empty) todaysClasses.innerHTML = "<p>No classes scheduled for today</p>";
...
Object.entries(map).forEach(r => rows.push(r));
...
snap.forEach(d => {
correctionRequests.innerHTML +=    <p>${d.data().studentId}   <button onclick="approveCorrection('${d.id}')">Approve</button></p>;
});
}
...
try {
// Get all classes for student's department & year
...
statusEl.innerHTML = `  
  Attendance: <b>${percentage}%</b><br>  
  Status: <b>${prediction}</b>  
`;  
